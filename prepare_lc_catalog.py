...
def main(sim_name, stem):
    ...
    for redshift in redshifts:
        ...
        np.savez(mock_sub_dir / f"galaxies_{tracer}_pos.npz", RA=RA, DEC=DEC, CZ=CZ, CZ_RSD=CZ_rsd, Z=Z, Z_RSD=Z_rsd, VEL=gals_real_vel, VEL_RSD=gals_rsd_vel, POS=gals_real_pos, POS_RSD=gals_rsd_pos, UNIT_LOS=gals_norm)
        np.savez(mock_sub_dir / f"randoms_{tracer}_pos.npz", RAND_RA=rands_RA, RAND_DEC=rands_DEC, RAND_CZ=rands_CZ, RAND_Z=rands_Z, RAND_POS=rands_pos)
