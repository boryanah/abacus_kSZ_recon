...
def get_z_comov(tracer):
    ...
    bias_fn = f"bias_{fn.split('/')[-1].split('.')[0]}{perc_str}.npz"
    data = np.load(com_fn)
...
def get_bias(z_binc, tracer, extra):
    ...
    for i in range(len(z_binc)):
        z = z_binc[i]
        print("redshift", z)
        fn = f"/pscratch/sd/b/boryanah/AbacusHOD_scratch/mocks_{box_lc}_output_kSZ_recon{extra}/AbacusSummit_base_c000_ph004/z{z:.3f}/galaxies/{tracer}s.dat"
        f = ascii.read(fn)
        mass = f['mass'] # Msun/h
...
for tracer in tracers:
    ...
    n_fun = interp1d(z_binc, comov_dens)
